...
**As-built authority**: The codebase is what runs today; it is **not** SSOT for the cross-feature domain model (constitution v2.9). Convergence **measures** code against spec/plan/contracts and appends tasks to align them — it does not treat code as author of spec Domain Mapping or `INV-*`. **`domain:check-converge` does not validate browser DOM interactivity** (e.g. chat inputs); if `quickstart.md` requires keystroke acceptance for an app route, verify manually or append a task when unmet.
...
- DOMAIN MODEL = `.specify/domain/domain.graph.yaml` (via `npm run domain:extract` from specs)
...
**From other specs' Domain Mapping:**

- Global `INV-*` invariants and entity ownership for cross-feature drift checks (not field-level SSOT); compiled in `.specify/domain/domain.graph.yaml` from spec Domain Mapping

**From feature data-model.md (if present):**

- Entity fields, tables, and feature-local invariants — authoritative for implementation gap checks

**From feature contracts/ (if present):**

- Route/API behavior and **`### Test contract`** sections naming saga module + `*.test.ts` (converge stage)
- Process edge IDs and compensations live in **`spec.md` § Saga and state machines** (specify stage; extracted to `domain.graph.yaml`)
- For each documented edge ID, expect a matching test case; missing coverage is a **`partial`** gap on the process invariant
...
**Technical saga pass** (when a plan contract has **`### Test contract`** naming a saga module):

1. Collect transition edge IDs from **`spec.md` § Saga and state machines** (or equivalent process section).
2. Read the **Test contract** in `contracts/*.md` for the `*.test.ts` module path.
3. If a test module is named, verify each edge ID has a test (by convention: `it(\`${edgeId} ...\`)` or `getTransitionById(edgeId)` loop over `AUTH_SAGA_EDGE_IDS` / equivalent).
4. Emit **`partial`** findings for undocumented edges in code or missing tests for documented edges.
5. Append remediation tasks: implement or extend the saga module + one test per edge; confirm `npm run domain:check-converge` passes (or `npm run domain:check-all` for full audit). If **`spec.md`** changed (semantic edit — Domain Mapping, Route/Syndication policy, Saga), re-run `domain:extract` + `domain:check` first.
